	...
	public RGBColor pathShade(ShadeRec rec, int depth) {
		...
		for(int i=0;i<lightnum;i++){
			...
			if(cur instanceof AreaLight){
				...
				if(!shadowed){
					...
					if(dotr>0.0)
						L.plusEqual(specular.get().multiply(cur.radiance(rec).multiply(dot*ksr*arealight.G(rec))));
				}
			}
		...
		if(pdfd>0.001) { // for rays going bak into objekt{
			...
			if(prob<0.5)
				...
				L.plusEqual(specular.get().multiply(rec.w.tracer.trace_ray(new Ray(rec.local_hit_point,r), depth+1,false)).multiply(1/(0.5*faktor)));
